...
from services.masking import PIIMasker, extract_document_header
...
class TestMaskingService(unittest.TestCase):
    ...
    def test_blocklist_nosta(self):
        ...
        # Should be replaced by {{COMPANY}} or similar via BLOCKED_COMPANY pattern
        self.assertNotIn("Nosta GmbH", masked)
        self.assertTrue("{{COMPANY}}" in masked or "{{PII_REDACTED}}" in masked)
